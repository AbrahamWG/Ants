...
class Ant(Insect):
    ...
    def add_to(self, place: Place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):  # self is not container
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self) -> Bee | None:
        ...
        if self.place.bees:  # start from current place
            return random_bee(self.place.bees)
        current = self.place
        current_distance = 0
        while current.entrance and not current.is_hive:
            if current.bees and (current_distance >= self.lower_bound and current_distance <= self.upper_bound):
...
class WallAnt(Ant):
    """Defensive ant with high health that blocks bee advancement.
    
    Acts as a defensive barrier with no attack capabilities but high durability.
    Useful for protecting more valuable ants behind it.
    """
    name = 'Wall'
    implemented = True
    food_cost = 4
...
class HungryAnt(Ant):
    """HungryAnt eats a random bee in its place, then needs time to chew.
    
    After eating a bee, the HungryAnt must spend time chewing before
    it can eat again.
    """
    name = 'Hungry'
    implemented = True
...
class TankAnt(ContainerAnt):
    """TankAnt is a container that also deals damage to all bees in its place."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a waterproof ThrowerAnt that can be placed in water."""
...
class QueenAnt(ThrowerAnt):
    ...
    The QueenAnt doubles the damage of all ants behind her in the tunnel.
    If the QueenAnt dies, the game is lost.
    """
    name = 'Queen'
    food_cost = 7
    implemented = True
